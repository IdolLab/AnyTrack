...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='anytrack', choices=['anytrack'],
                        help='training script name')
    parser.add_argument('--config', type=str, default='anytrack', help='yaml configure file name')
    args = parser.parse_args()

    return args


def evaluate_vit(model, template, search, pre_box):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search, pre_box),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search, pre_box)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search, pre_box)
        torch.cuda.synchronize()
