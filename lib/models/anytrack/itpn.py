...
from lib.models.anytrack.base_backbone import BaseBackbone
from transformers import BertTokenizer
from transformers.models.bert.modeling_bert import BertConfig, BertEmbeddings
from lib.models.anytrack import utils as utils
from lib.models.anytrack.utils import combine_tokens, recover_tokens, generate_template_mask
from lib.models.layers.cross_layer import *
from lib.models.layers.DGMI import DynamicGraphModalityInteraction
...
class Fast_iTPN(BaseBackbone):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        self.DGMI = nn.ModuleList()
        if self.cross_loc is not None and type(self.cross_loc) == list:
            for i in range(len(self.cross_loc)):
                self.DGMI.append(
                    DynamicGraphModalityInteraction(d_model=embed_dim, num_heads=num_heads, num_layers=2, dropout=0.))
    ...
    def _z_feat(self, z, num_modalities):
        z = torch.stack(z, dim=1)
        B, T_z, C_z, H_z, W_z = z.shape
        ...
        for mod_idx in range(num_modalities):
            mod_z = z[:, mod_idx * 3:(mod_idx + 1) * 3, :, :]
            mod_z = self.patch_embed(mod_z)
            ...
            # 展平并添加位置嵌入
            mod_z = mod_z.flatten(2).transpose(1, 2)
            mod_z += self.pos_embed_z
            modalities_features.append(mod_z)

        # 如果有多于一个模板帧，则调整形状
        if T_z > 1:  # multiple memory frames
            for i in range(len(modalities_features)):
                mod_feat = modalities_features[i]
                mod_feat = mod_feat.view(B, T_z, -1, mod_feat.size()[-1]).contiguous()
                mod_feat = mod_feat.flatten(1, 2)
                modalities_features[i] = mod_feat

        return modalities_features  # 返回包含所有模态特征的列表

    def _x_feat(self, x, num_modalities):

        # 处理每个模态
        modalities_features = []
        for mod_idx in range(num_modalities):
            mod_x = x[:, mod_idx * 3:(mod_idx + 1) * 3, :, :]
            mod_x = self.patch_embed(mod_x)

            if not self.convmlp and self.stop_grad_conv1:  # self.convmlp==True
                mod_x = mod_x.detach() * 0.9 + mod_x * 0.1
                assert self.convmlp == True, '想像失败'

            for blk in self.blocks[:-self.num_main_blocks]:
                mod_x = blk(mod_x)

            mod_x = mod_x.flatten(2).transpose(1, 2)
            mod_x += self.pos_embed_x
            modalities_features.append(mod_x)

        return modalities_features  # 返回包含所有模态特征的列表

    def _fusion_feat(self, z_modalities, x_modalities, B, num_modalities, temporal_query, tau, len_zx, number):
        modality_features = []
        for mod_idx in range(num_modalities):
            modality = combine_tokens(z_modalities[mod_idx], x_modalities[mod_idx], mode=self.cat_mode)
            if self.add_cls_token:
                ...
                if temporal_query is None:
                    ...
                    if temporal_query[mod_idx] is None:
                        temporal_init = temporal_init + self.temporal_pos_embed
                    else:
                        temporal_init = temporal_query[mod_idx] + temporal_init + self.temporal_pos_embed
            modality = torch.cat([temporal_init, modality], dim=1)
            modality = self.pos_drop(modality)
            modality_features.append(modality)
        ...
        for layer_idx, blk in enumerate(self.blocks[-self.num_main_blocks:]):
            ...
            for i in range(num_modalities):
                modality_features[i], _ = blk(modality_features[i])
            # 跨模态交互
            if self.cross_loc is not None and layer_idx in self.cross_loc and num_modalities > 1:
                modality_features, l_dag = self.DGMI[cross_index](modality_features, tau, len_zx, number)
                l_dag_list.append(l_dag)
                cross_index += 1

        fused_features = torch.cat(modality_features, dim=1)
        fused_features = self.norm(fused_features)

        return fused_features, l_dag_list
    ...
    def forward_features(self, z, x, track_query, tau):
        B = x.shape[0]
        number = len(z)
        ...
        if total_channels % 3 != 0:
            raise ValueError(f"输入通道数({total_channels})必须是3的倍数")
        num_modalities = total_channels // 3

        # 获取多模态特征
        z_modalities = self._z_feat(z, num_modalities)  # 返回模态特征列表
        x_modalities = self._x_feat(x, num_modalities)  # 返回模态特征列表
        fusion_feat, l_dag_list = self._fusion_feat(z_modalities, x_modalities, B, num_modalities, track_query, tau, len_zx, number)

        aux_dict = {"l_dag_list": l_dag_list,
                    }

        return fusion_feat, aux_dict, len_zx

    def forward(self, z, x, track_query, tau, **kwargs):
        """
        ...
        Returns:
            ...
            attn : None
        """
        x, aux_dict, len_zx = self.forward_features(z, x, track_query, tau)
