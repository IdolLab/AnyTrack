...
from lib.train.actors import ANYTRACKActor
# for import modules
import importlib
...
def run(settings):
    settings.description = 'Training script for anytrack'
    ...
    # Create network
    if settings.script_name == "anytrack":
        net = build_anytrack(cfg)
    else:
    ...
    if settings.script_name == "anytrack":
        ...
        actor = ANYTRACKActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
