...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        ...
        # 支持1-5个模态
        if not 1 <= num_modalities <= 5:
            raise ValueError(f"当前支持1-5个模态，但检测到{num_modalities}个模态")

        # 动态分割和归一化每个模态
        normalized_modalities = []
        for i in range(num_modalities):
            modality = image[i * 3:(i + 1) * 3, :, :]
            if i == 0:  # 第一个模态使用base mean和std
                normalized = tvisf.normalize(modality, self.mean_base, self.std_base, self.inplace)
            else:  # 其他模态使用others mean和std
                normalized = tvisf.normalize(modality, self.mean_others, self.std_others, self.inplace)
            normalized_modalities.append(normalized)

        return torch.cat(normalized_modalities, 0)
