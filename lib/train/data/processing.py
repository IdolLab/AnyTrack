...
class ViPTProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['template', 'search']:
            ...
            # 如果存在前一帧bbox（仅针对search），也进行相同的处理
            if s == 'search' and 'search_prev_anno' in data:
                _, prev_boxes, _, _ = prutils.jittered_center_crop(data[s + '_images'], jittered_anno,
                                                                           data['search_prev_anno'],
                                                                           self.search_area_factor[s],
                                                                           self.output_sz[s])
                # 只对bbox进行transform，不处理图像
                _, data['search_prev_anno'] = self.transform[s](image=crops, bbox=prev_boxes, joint=False)
