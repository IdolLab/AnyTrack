...
def get_rgbdt_frame(color_path, depth_path, infrared_path, gray_path, depth_clip=False):
    if color_path:
    ...
    img = cv2.merge((rgb, gray, colormap, infrared))
    return img
...
class RGBDT(BaseVideoDataset):
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        '''
        return os.path.join(seq_path, 'color', '{:08}.png'.format(frame_id+1)) , os.path.join(seq_path, 'depth', '{:08}.png'.format(frame_id+1)), os.path.join(seq_path, 'infrared', '{:08}.png'.format(frame_id+1)), os.path.join(seq_path, 'gray', '{:08}.png'.format(frame_id+1))
    ...
    def _get_frame(self, seq_path, frame_id):
        ...
        Return :
            ...
            - raw_depth
        '''
        color_path, depth_path, infrared_path, gray_path = self._get_frame_path(seq_path, frame_id)
        img = get_rgbdt_frame(color_path, depth_path, infrared_path, gray_path, depth_clip=True)
