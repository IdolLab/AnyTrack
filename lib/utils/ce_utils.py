...
def cosine_anneal_tau(epoch, total_anneal_epochs, tau_init=1.0, tau_final=0.1):
    """
    余弦退火：仅在 total_anneal_epochs 内退火，之后保持 tau_final
    """
    if epoch < total_anneal_epochs:
        cos_inner = math.pi * (epoch / total_anneal_epochs)
        return tau_final + 0.5 * (tau_init - tau_final) * (1 + math.cos(cos_inner))
    else:
        return tau_final
